// Serial C++ translation of SimpleRenderWithSTL.py
// Reads an ASCII STL file, ray-traces one ray per pixel (Moller-Trumbore
// intersection + Lambert diffuse shading) and writes the result to a PPM file.
// Faithful 1:1 port of the Python reference -- no matplotlib display.
...
// scalar * Vec3 (mirrors Python __rmul__)
...
struct Triangle {
    ...
    std::optional<Hit> intersect(const Vec3& C, const Vec3& D) const {
        ...
        if (-eps < beta && beta < eps) return std::nullopt;
...
struct Scene {
    ...
    Vec3 trace(const Vec3& C, const Vec3& D, const Vec3& Light) const {
        ...
        if (!closest) return Vec3(1.0, 1.0, 1.0);
...
std::vector<Triangle> load_stl(const std::string& path, const Vec3& color) {
    ...
    std::ifstream f(path);
    if (!f) throw std::runtime_error("STL file not found");
